...
class App {
    ...
    public function run(Request $request = null) {
        ...
        try {
            ...
            // 监听 app_dispatch
            Hook::listen('app_dispatch', $this->dispatch);
            // 获取应用调度信息
            ...
            //throw new HttpException(404, json_encode($dispatch));
            // 记录当前调度信息
            ...
            // 请求缓存检查
            $request->cache(
                    $config['request_cache'], $config['request_cache_expire'], $config['request_cache_except']
            );
            $data = $this->exec($dispatch, $config);
        ...
        if (defined('RPC_RUN') && RPC_RUN == true) {
            return $data; //wang change rpc
        }
    ...
    public function initCommon() {
        if (empty($this->init)) {
            ...
            Loader::addNamespace($this->namespace, APP_PATH);
            ...
            // 应用调试模式
            $this->debug = Env::get('app_debug', Config::get('app_debug'));
    ...
    private function bindParams($reflect, $vars = []) {
        ...
        if ($reflect->getNumberOfParameters() > 0) {
            ...
            foreach ($reflect->getParameters() as $param) {
                $args[] = $this->getParamValue($param, $vars, $type);
            }
        }
     ...
     * @return array
     */
    private function getParamValue($param, &$vars, $type) {
        $name = $param->getName();
     ...
     */
    protected function exec($dispatch, $config) {
        switch ($dispatch['type']) {
            ...
            case 'module': // 模块/控制器/操作
                $data = $this->module($dispatch['module'], $config, isset($dispatch['convert']) ? $dispatch['convert'] : null);
                break;
            case 'controller': // 执行控制器操作
                $vars = array_merge(Request::instance()->param(), $dispatch['var']);
                $data = Loader::action(
                                $dispatch['controller'], $vars, $config['url_controller_layer'], $config['controller_suffix']
                );
                break;
            case 'method': // 回调方法
                $vars = array_merge(Request::instance()->param(), $dispatch['var']);
     ...
     */
    public function module($result, $config, $convert = null) {
        if (is_string($result)) {
        ...
        if ($config['app_multi_module']) {
            ...
            if ($bind) {
                ...
                $available = true;
            } else if (defined('RPC_RUN') && RPC_RUN == true) {
                $available = true; //wang change
            }
            // 模块初始化
            if ($module && $available) {
                // 初始化模块
        ...
        try {
            $instance = Loader::controller(
                            $controller, $config['url_controller_layer'], $config['controller_suffix'], $config['empty_controller']
            );
    ...
    public function routeCheck($request, array $config) {
        $path = $request->path();
        //throw new \think\exception\HttpException(404, 'path2:'.$path);//okkkk
