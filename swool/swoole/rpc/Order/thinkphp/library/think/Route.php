...
class Route
{
    ...
    {
        //检查解析缓存
        if (!(new App)->debug && Config::get('route_check_cache')) {
    ...
    private static function checkUrlBind(&$url, &$rules, $depr = '/')
    {
        if (!empty(self::$bind)) {
            ...
            $bind = self::$bind[$type];
            // 记录绑定信息
            (new App)->debug && Log::record('[ BIND ] ' . var_export($bind, true), 'info');
            // 如果有URL绑定 则进行绑定检测
            switch ($type) {
                case 'class':
                    // 绑定到类
    ...
    public static function parseUrl($url, $depr = '/', $autoSearch = false)
    {
        ...
        if (isset($path)) {
            ...
            if ($autoSearch) {
                ...
                $suffix = (new App())->suffix  || Config::get('controller_suffix') ? ucfirst(Config::get('url_controller_layer')) : '';
                $item   = [];
                $find   = false;
                foreach ($path as $val) {
                    $item[] = $val;
    ...
    private static function parseRule($rule, $route, $pathinfo, $option = [], $matches = [], $fromCache = false)
    {
        ...
        if ($route instanceof \Closure) {
            ...
            $a = new App();
            $a->modulePath = APP_PATH . (Config::get('app_multi_module') ? $request->module() . DS : '');
        } else {
            // 路由到模块/控制器/操作
