- [微服务架构](#微服务架构)
- [积累](#积累)
- [1.初识Sentinel](#1初识sentinel)
  - [1.1.雪崩问题及解决方案](#11雪崩问题及解决方案)
    - [1.1.1.雪崩问题](#111雪崩问题)
    - [解决方案](#解决方案)
    - [1.1.2.超时处理](#112超时处理)
    - [1.1.3.仓壁模式](#113仓壁模式)
    - [1.1.4.断路器--熔断降级](#114断路器--熔断降级)
    - [1.1.5.限流](#115限流)
    - [1.1.6.总结](#116总结)
  - [1.2.服务保护技术对比](#12服务保护技术对比)
  - [1.3.Sentinel介绍和安装](#13sentinel介绍和安装)
    - [1.3.1.初识Sentinel](#131初识sentinel)
    - [1.3.2.安装Sentinel](#132安装sentinel)
  - [1.4.微服务整合Sentinel](#14微服务整合sentinel)
- [2.流量控制](#2流量控制)
  - [概念理解](#概念理解)
  - [2.1.簇点链路](#21簇点链路)
  - [2.1.快速入门](#21快速入门)
    - [2.1.1.示例](#211示例)
    - [介绍一款**压力测试工具：jmeter**](#介绍一款压力测试工具jmeter)
    - [2.1.2.练习：](#212练习)
      - [监控报告](#监控报告)
  - [2.2.流控模式](#22流控模式)
    - [2.2.1.关联模式](#221关联模式)
    - [2.2.2.链路模式](#222链路模式)
      - [1）添加查询商品方法](#1添加查询商品方法)
      - [2）查询订单时，查询商品](#2查询订单时查询商品)
      - [3）新增订单，查询商品](#3新增订单查询商品)
      - [4）给查询商品添加资源标记](#4给查询商品添加资源标记)
      - [5）添加流控规则](#5添加流控规则)
      - [6）Jmeter测试](#6jmeter测试)
    - [2.2.3.总结](#223总结)
  - [2.3.流控效果](#23流控效果)
    - [2.3.1.warm up](#231warm-up)
      - [1）配置流控规则：](#1配置流控规则)
      - [2）Jmeter测试](#2jmeter测试)
    - [2.3.2.排队等待](#232排队等待)
      - [1）添加流控规则](#1添加流控规则)
      - [2）Jmeter测试](#2jmeter测试-1)
    - [2.3.3.总结](#233总结)
  - [2.4.热点参数限流](#24热点参数限流)
    - [2.4.1.全局参数限流](#241全局参数限流)
    - [2.4.2.热点参数限流](#242热点参数限流)
    - [2.4.4.案例](#244案例)
      - [1）标记资源](#1标记资源)
      - [2）热点参数限流规则](#2热点参数限流规则)
      - [3）Jmeter测试](#3jmeter测试)
- [3.隔离和降级](#3隔离和降级)
  - [3.1.FeignClient整合Sentinel（实现降级效果）](#31feignclient整合sentinel实现降级效果)
    - [3.1.1.修改配置，开启sentinel功能](#311修改配置开启sentinel功能)
    - [3.1.2.编写失败降级逻辑](#312编写失败降级逻辑)
    - [3.1.3.总结](#313总结)
  - [3.2.线程隔离（舱壁模式）](#32线程隔离舱壁模式)
    - [3.2.1.线程隔离的实现方式](#321线程隔离的实现方式)
    - [3.2.2.sentinel的线程隔离](#322sentinel的线程隔离)
      - [1）配置隔离规则](#1配置隔离规则)
      - [2）Jmeter测试](#2jmeter测试-2)
    - [qps和线程数的区别](#qps和线程数的区别)
    - [3.2.3.总结](#323总结)
  - [3.3.熔断降级](#33熔断降级)
    - [3.3.1.慢调用](#331慢调用)
      - [1）设置慢调用](#1设置慢调用)
...
- [4.授权规则](#4授权规则)
  - [4.1.授权规则](#41授权规则)
    - [4.1.1.基本规则](#411基本规则)
    - [4.1.2.如何获取origin](#412如何获取origin)
    - [4.1.3.给网关添加请求头](#413给网关添加请求头)
    - [4.1.4.配置授权规则](#414配置授权规则)
  - [4.2.自定义异常结果](#42自定义异常结果)
    - [4.2.1.异常类型](#421异常类型)
    - [4.2.2.自定义异常处理](#422自定义异常处理)
- [5.规则持久化](#5规则持久化)
  - [5.1.规则管理模式](#51规则管理模式)
    - [5.1.1.pull模式](#511pull模式)
    - [5.1.2.push模式](#512push模式)
  - [5.2.实现push模式](#52实现push模式)

# 微服务架构

![alt text](image-1.png)
1. 微服务之间的调用：同步调用的话可以用feign调用，异步调用的话可以用消息队列kafka调用
   1. 依赖返回结果的用同步，不依赖调用结果的只需要把消息发出去就行的用异步消息队列
2. 用feign远程调用微服务应用时，如果对方微服务应用宕机了，或一个请求执行10秒，并发请求时崩溃了怎么办，需要对微服务做些保护的手段

# 积累

1. 介绍一款压力测试工具：jmeter
   1. 参考链接https://juejin.cn/post/7304271679009341449
...
因为运维任意是知道自己的机器能力最大能支持多少并发请求，所以需要根据自己的机器能力来提前设置限流的阈值。避免过载情况发生
...
- [Netfix Hystrix](https://github.com/Netflix/Hystrix): 主要作服务保护，提供服务降级、服务熔断、服务限流等功能。

- [Sentinel](https://github.com/alibaba/Sentinel)：阿里巴巴开源的服务保护框架。
...
我们微服务项目中有多个微服务应用，每个应用都有自己的Sentinel控制台。我们先在order-serivce微服务应用中整合sentinel。
...
--- 代码中无需额外配置，Sentinel 会在适配的spring框架中自动注册拦截器，对所有 @RequestMapping 标记的接口埋点，只要有接口访问，就触发埋点规则，触发监控的资源收集监控
...
## 概念理解
在 Sentinel 中，簇点链路（Cluster Point Link） 是对系统中所有资源（被保护的对象）及其调用关系的可视化展示。其中，端点（Endpoint） 和 资源（Resource） 是两个核心概念

1. 簇点链路：当请求进入微服务时，首先会访问DispatcherServlet，然后进入Controller、Service、Mapper，这样的一个调用链就叫做**簇点链路**。簇点链路中被监控的每一个接口就是一个**资源**。
   1. 簇点链路会按调用关系展示资源的层级，例如：
/api/order/create（端点 / 资源） → createOrder（资源，注解指定） → payService:pay（资源，手动标记）
2. 资源：是 Sentinel 保护的核心对象，可以是任何需要被限流、熔断、降级的 “业务单元”。它是一个抽象概念，不局限于特定类型，只要通过 Sentinel API 标记过的对象，都可称为资源
   1. 所有流量控制逻辑都围绕资源展开
3. 端点：通常指 HTTP 接口（或 Dubbo 等服务接口）的路径，是资源的一种 “具象化表现”。在 Web 应用中，每个 HTTP 接口（如 /api/order/create）会被自动识别为一个端点，同时也会被 Sentinel 自动标记为一个资源
4. 关系与区别：资源是抽象，端点是特殊资源
   1. 包含关系：端点属于资源的一种，所有端点都是资源，但资源不一定是端点（资源可以是方法、代码块等）。
   2. 创建方式：端点由 Sentinel 自动创建（针对 HTTP/Dubbo 接口），资源可手动创建（针对任意业务单元）。
   3. 作用范围：资源覆盖范围更广（接口、方法、代码块等），端点仅针对接口级别的资源。
...
点击端点资源/order/{orderId}后面的流控按钮，就可以弹出表单。
...
这就是限流的效果：限流就是直接阻断请求，直接返回错误提示
![alt text](image-3.png)

### 介绍一款**压力测试工具：jmeter**
- 参考链接https://juejin.cn/post/7304271679009341449
...
- 线程数：代表模拟多少用户数量发起请求
- rameup：几秒内启动完成这些线程数发出请求
- 循环次数：代表每个线程执行的次数
- 上图就意思代表：20个用户，2秒内启动线程发出，QPS是10，超过了5.
...
**语法说明**：当/write资源访问量触发阈值时，就会对/read资源限流，避免影响/write资源。（直接对/read限流即阻断访问）



**使用场景**：比如用户支付时需要修改订单状态，同时用户要查询订单。查询和修改操作会争抢数据库锁，产生竞争。业务需求是优先支付和更新订单的业务，因此当修改订单业务触发阈值时，需要对查询订单业务限流。

- 一般都是两个业务场景竞争比较大的场景，才会使用关联模式，如
...
**意义**：显然是创建订单的优先级更高，所以并发量高时，适当给查询订单进行限流
...
阈值一般是一个微服务能承担的最大QPS，但是一个服务刚刚启动时，一切资源尚未初始化（**冷启动**），此时的并发承受能力比较弱，还不能承受极限，需要慢慢预热才行，所以有这个预热模式，慢慢的把QPS从一个适合当前服务能力的可控较小值逐渐增加到最大阈值。

- 举个糟糕例子：如果此时冷启动时，还没预热好，直接接收高并发的请求，服务会承受不住，导致服务崩溃宕机。
- 可控较小值：冷启动时，先把qps调低些，稍微多点的请求就直接失败回复，等服务预热好，再把qps调大。
...
- 应用场景为什么需要？：
  - 促销活动（如秒杀、限时折扣）中，流量具有明显的 “倾斜性”，少数几款热门商品（如 “9.9 元秒杀手机”）的请求量可能占总流量的 80% 以上；
  - 普通的全局限流（如限制接口总 QPS 为 1000）无法解决此问题 —— 可能热门商品已被请求 900 次（接近阈值），而非热门商品只能分配到 100 次请求，导致资源分配失衡
    - 改成热门商品限流为500，非热门商品限流为100

  - 作用：资源精准保护：只限制热门商品的请求，不影响非热门商品的正常访问，避免 “一刀切”；
...
- 降级：熔断了直接返回失败信息不太友好，可以采用降级方案，比如查询订单，当查询服务熔断挂了时，可以走个降级策略如返回一些可能感兴趣的推荐商品
...
## 3.1.FeignClient整合Sentinel（实现降级效果）
...
在传统的同步 Web 服务器（如 Tomcat、Nginx 同步模式）中，一个请求会对应一个处理线程：服务器接收到请求后，从软件线程池分配一个线程处理该请求，直到请求完成（包括业务逻辑、IO 操作等），线程才会释放回池。这种情况下，“请求数” 和 “线程数” 在并发场景下基本对应。

但在以下场景中，“请求” 和 “线程” 不一一对应：

1. 异步处理场景：例如使用 Spring WebFlux、Netty 等异步框架时，一个线程可以处理多个请求（通过事件驱动、非阻塞 IO 实现），此时请求数会远大于线程数。
2. 协程场景：在支持协程的语言（如 Go）或框架中，多个协程（轻量级 “线程”）可以运行在一个操作系统线程上，一个请求可能由一个协程处理，而多个请求的协程共享一个线程
...
比如场景：微服务都是由网关作为统一入口的，万一调用者直接访问微服务应用进行请求，是不是微服务也得需要做下来源授权控制，允许或不允许的来源规则
