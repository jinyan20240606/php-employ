...
- [Sentinel 规则持久化](#sentinel-规则持久化)
  - [一、修改order-service服务](#一修改order-service服务)
    - [1.引入依赖](#1引入依赖)
    - [2.配置nacos地址](#2配置nacos地址)
  - [二、修改sentinel-dashboard源码](#二修改sentinel-dashboard源码)
    - [1. 解压](#1-解压)
    - [2. 修改nacos依赖](#2-修改nacos依赖)
    - [3. 添加nacos支持](#3-添加nacos支持)
    - [4. 修改nacos地址](#4-修改nacos地址)
    - [5. 配置nacos数据源](#5-配置nacos数据源)
    - [6. 修改前端页面](#6-修改前端页面)
    - [7. 重新编译、打包项目](#7-重新编译打包项目)
    - [8.启动](#8启动)
...
java -jar -Dnacos.addr=139.196.105.154:58848 sentinel-dashboard.jar
```
