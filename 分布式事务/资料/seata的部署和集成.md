...
# 数据卷挂载：将宿主机的 seata-config 数据卷（持久化存储）挂载到容器内的 /seata-server/resources 目录（Seata 配置文件目录，用于持久化自定义配置，避免容器删除后配置丢失）

# 配置会持久化到宿主机的 /var/lib/docker/volumes/seata-config/_data 目录，后续修改配置可直接操作宿主机该目录下的文件，无需进入容器
```
