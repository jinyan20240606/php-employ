...
- [1.2  多表查询](#12--多表查询)
    ...
    - [1.2.4  交叉连接](#124--交叉连接)
    - [1.2.5  自然连接](#125--自然连接)
    - [1.2.6  using](#126--using)
    - [1.2.7  练习](#127--练习)
- [1.3  子查询](#13--子查询)
    - [1.3.1  标量子查询（最常用的）](#131--标量子查询最常用的)
    - [1.3.2  列子查询](#132--列子查询)
    - [1.3.3  行子查询](#133--行子查询)
    - [1.3.4  表子查询（面试常考）](#134--表子查询面试常考)
    - [1.3.5  exists子查询](#135--exists子查询)
- [1.4  视图](#14--视图)
    - [1.4.1  概述](#141--概述)
    - [1.4.2  作用](#142--作用)
    - [1.4.3  创建视图](#143--创建视图)
...
-- 语法二
select * from 表1，表2 where 表1.公共字段=表2.公共字段 // 不加where就会返回笛卡尔积
...
-- 相同的字段只显示一次（解决上面stuNo显示了2次重复）
mysql> select s.stuno,stuname,stusex,writtenexam,labexam from stuinfo s inner join stumarks m on s.stuno=m.stuno; // from stuinfo s 是省略的as，给stuinfo表起的别名。select s.stuno是因为stuno是公共字段不知道指定的是哪个字段，所以要表名点否则会报错
+--------+----------+--------+-------------+---------+
...
自动判断条件连接，判断的条件是依据同名字段，是用来简化上面的内外链接的

1、自然内连接（natural join）

```mysql
mysql> select * from stuinfo natural join stumarks;  // 自动判断两表中的相同字段
+--------+----------+--------+--------+---------+------------+---------+-------------+---------+
...
```mysql
mysql> select * from stuinfo natural left join stumarks;  // 自动判断两表中的相同字段
+--------+----------+--------+--------+---------+------------+---------+-------------+---------+
| stuNo  | stuName  | stuSex | stuAge | stuSeat | stuAddress | examNo  | writtenExam | labExam |
+--------+----------+--------+--------+---------+------------+---------+-------------+---------+
| s25301 | 张秋丽         | 男       |     18 |       1 | 北京           | s271816 |          77 |      82 |
...
```mysql
mysql> select * from stuinfo natural right join stumarks;  // 自动判断两表中的相同字段
+--------+---------+-------------+---------+----------+--------+--------+---------+------------+
...
万一前面自然链接自动选同名字段的时候，万一遇到多个同名字段时，该自动选哪个？？？
- 解法：using用来指定连接字段，也可以用在非自然链接里
...
上面是2个表，stuinfo学生信息表和stumarks成绩表

1、显示地区及每个地区参加笔试的人数，并按人数降序排列

```mysql
-- 第一步： 显示地区及每个地区参加笔试的人数（左外链接）
...
子查询为父查询提供查询条件，可用于多个表联合查的情况直接嵌套查，也可以用于单个表



#### 1.3.1  标量子查询（最常用的）

特点：子查询返回的值是一个值用,等号赋值(子查询结果是**1列1行**就只返值)。

```mysql
-- 查找笔试成绩是80的学生（子查询从成绩表查出学号，直接赋值到学生信息查询语句的where条件中）
...
注意：如果子查询的结果需要返回多条记录, 就使用下面讲的列子查询



#### 1.3.2  列子查询

特点： 子查询返回的结果是**一列多行**（1个字段多条记录就是select后面只跟1个字段）
...
特点：子查询返回的结果是多个字段组成（子查询结果**多列1行**）

```mysql
-- 查找语文成绩最高的男生和女生（先子查询查性别对应最高的语文成绩一行值，再用性别对应的最高成绩，查出最高成绩和性别对应的学生具体信息）
...
特点：将子查询的结果作为表（子查询结果是**多行多列**多行多列就形成个表的结构了）

```mysql
-- 查找语文成绩最高的男生和女生 (利用了分组字段普通字段按第一个为准的特性，自动选出了男女语文最高的记录)
...
当这个表想增加权限时，对不同的开发者的权限级别暴漏不同的字段（不想把表的全部结构暴漏给别人），不同的权限级别访问不同的字段
...
2、视图只是个表结构，视图中并不存放数据，而是存放在视图所引用的原始表（基表）中
...
- 小结：
  - information_schema是数据库的大脑，只要装好数据库自动就带了的，存着所有数据库相关东西包括视图
  - 在当前数据库中查找其他数据库中的表名信息：
...
方法一：利用了分组字段普通字段按第一个为准的特性，自动选出了男女语文最高的记录
...
```mysql
mysql> create view view3    -- 第一步
    -> as
    -> select * from stu order by ch desc;
Query OK, 0 rows affected (0.00 sec)

mysql> select * from view3 group by stusex;  -- 第二步
+--------+---------+--------+--------+---------+------------+------+------+
| stuNo  | stuName | stuSex | stuAge | stuSeat | stuAddress | ch   | math |
+--------+---------+--------+--------+---------+------------+------+------+
...
1、merge：合并算法（将视图语句和外层语句合并后再执行）
    （1）方法1是：先执行里面的，再执行外层的表查询
    （2）方法2是：视图里面的和外面的合并后再执行


2、temptable:临时表算法（将视图作为一个临时表来执行：先执行视图再执行外层）
...
```mysql
-- 创建视图，指定算法为临时表算法
mysql> create or replace algorithm=temptable view view3  -- 如果没有就创建，有就替换
    -> as
    -> select * from stu order by ch desc;
Query OK, 0 rows affected (0.00 sec)
...
事务(TRANSACTION)是一个整体，职责单一的工作单元，要么一起执行，要么一起不执行
...
一致性（Consistency）：当事务完成时，数据必须处于一致状态
    -- 体现在两个客户端窗口操作数据库时，某个窗口的事务完成后，另个窗口的数据也会同步一致的，
    -- 隔离性就是多个窗口的事务之间在未提交前都是临时隔离的
...
```mysql
start transaction  或 begin [work] // 从这回车后往下就看作一个事务整体
```
...
-- 开启事务
mysql> begin;      // 从这往下就看作1个整体
Query OK, 0 rows affected (0.00 sec)

mysql> delimiter //   -- 更改定界符，将来去写//时才去执行，以前是默认分号执行。
...
-- 提交事务：当提交事务后，再回滚就没用了回滚不回去，因为事务已经提交了就结束状态了，前面的能回滚是提交前的
mysql> commit //
Query OK, 0 rows affected (0.00 sec)
...
```
每一个SQL语句本质上都是一个独立的事务，要么都成功要么都失败
```
...
```
1、列中仅包含几个不同的值。（如性别列就男女有啥用）
2、表中仅包含几行。为小型表创建索引可能不太划算，因为MySQL在索引中搜索数据所花的时间比在表中逐行搜索所花的时间更长 
```

（对普通办公笔记本来说近8百万条数据，查询1s左右，创建索引后查询0.几秒，百毫秒级左右）
...
create table t5(
    id int primary key,
    name varchar(20),
    unique ix_name(name)	-- 添加唯一索引，索引名字是ix_name，关联字段是name，unique后面跟的index可以省略
...
mysql> create unique index ix_name on t5(name);  // 创建唯一索引，唯一索引的名字是ix_name在t5表的name字段上
...
```
-- 索引的使用: 当查询语言where name等于什么，就自动根据name的唯一索引去查了，最终查出那条索引所在的记录
mysql> select * from t7 where name = '张三';  
+----+-------------+-------+------+---------------+------+---------+------+------+----------------+
| id | select_type | table | type | possible_keys | key  | key_len | ref  | rows | Extra          |
+----+-------------+-------+------+---------------+------+---------+------+------+----------------+
|  1 | SIMPLE      | t7    | ref  | ix_name       | ix_name | 767     | 张三 |    1 | Using index    |
+----+-------------+-------+------+---------------+------+---------+------+------+----------------+
1 row in set (0.00 sec)
```
...
- round(3.14159,3):2参不写就是4舍五入取整，写了2参就是小数后几位四舍五入
...
mysql> select concat(stuname,'-',stusex) 信息 from stuinfo;   // 如果不写别名，字段名就默认是函数字符串
+-------------+
| 信息           |
+-------------+
| 张秋丽-男           |
...
-- 星期，本年第几天;
mysql> select dayname(now()) 星期,dayofyear(now()) 本年第几天; // 还有monthname(now()) 打印出月份英文名字
+-----------+------------+
| 星期          | 本年第几天          |
+-----------+------------+
| Wednesday |        163 |
...
存密码存明文你不就看到了吗，加个密存进去密码

1、md5()：md5类型：char(32)，不管加密什么都一定是32位，16进制表示，不可逆
...
每个sql语句代码的段的执行都要经历：词法分析——语法分析——编译——执行
...
-- 调用预处理，并传参数
mysql> delimiter //
mysql> set @id='s25301';  // sql语句中定义变量：必须是@开头
